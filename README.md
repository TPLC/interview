...
<details>
  ...
  - 也可以使用 gtest 进行测试，gtest提供两种类型的 `TEST` 宏，一个是 `ASSERT_` 宏，如果当前点检测失败则退出当前测试函数（*注意是退出函数，而不是退出程序，`assert` 则是直接退出程序*），一个是 `EXPECT_` 宏，如果当前点检测失败则继续往下执行。
  ```C++
  #include <iostream>

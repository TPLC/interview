...
<details>
  ...
  - **如何保证可靠数据传输**：参考了 rdt3.0 机制和 TCP 实现可靠数据传输的方式，主要在**应用层**使用序列号、超时重传（*发送方才有计时器*），缓冲区（*发送方才有缓冲区*）来实现可靠数据传输（*而 TCP 是在传输层实现可靠数据传输，程序员不需要干预*）。主控模块发送第一个报文段会包含一个初始序列号，然后主控模块每发送一个正常报文段都会将序列号加一，而且其他模块收到主控模块发送来的报文段后，要返回一个和主控模块报文段序列号一致的报文段给主控模块；当其他模块发现主控模块发送来的报文段校验不正确时，返回一个带有序列号的 NAK 报文段给主控模块，代表接收到的数据不正确，

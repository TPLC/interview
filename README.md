...
<details>
  ...
  - 使用 `sizeof` 看一个指针的大小是 4（32 位系统）或者 8 （64 位系统）；而引用则是被引用对象的大小。
...
<details>
  ...
  public:
      void set_ptr(shared_ptr<CA>& ptr) { m_ptr_a = ptr; }
  private:
      weak_ptr<CA> m_ptr_a; // 改成 weak_ptr，只要 m_ptr_b 和  m_ptr_a 其中一个是 weak_ptr 是智能指针就行
  };
...
<details>
  ...
  - **右值引用**：用于对右值的引用，通过 `&&` 来声明。在定义右值引用时，`=` 右边要求是一个右值。
    ...
    - **srd::forward()**：`srd::forward()` 的作用是保留变量的左右值属性。
...
<details>
  ...
  - **生长方向**：栈是从内存的高地址向内存的低地址扩展；堆是从内存的低地址向高地址扩展。
  - **申请效率**：栈是机器系统提供的数据结构，**计算机底层对栈提供支持**，比如分配专门的寄存去存放栈的地址，压栈和出栈都有专门的指令执行，所以栈的效率很高；**堆的分配是 C++ 函数库提供的**，机制很复杂，比如为了分配一块内存，库函数首先会搜索有没有足够大的空间，如果没有就调用系统功能增加程序数据段的内存空间然后返回，所以堆的效率很低。
  - **碎片问题**：栈的分配不会产生内存碎片；堆如果频繁的进行 `new` 和 `delete` 的话会造成内存空间的不连续，从而造成大量的内存碎片，使程序效率降低。
  - **存储内容**：栈中一般存放返回地址，局部变量，函数参数等；存放在堆中的一块数据一般会在头部存放该数据块的大小，堆中的具体内容由程序员决定。
  - **申请后系统的响应**（*非面试重点*）：申请栈上的空间时，只要栈的剩余空间大于所申请空间，系统将为程序提供内存，否则将报异常提示栈溢出；申请堆上的空间时，首先应该知道操作系统有一个记录空闲内存地址的链表，当系统收到程序的申请时，会遍历该链表，寻找第一个空间大于所申请空间的堆结点，然后将该结点从空闲结点链表中删除，并将该结点的空间分配给程序，另外，对于大多数系统，会在这块内存空间中的首地址处记录本次分配的大小，这样，代码中的 `delete` 语句才能正确的释放本内存空间。另外，由于找到的堆结点的大小不一定正好等于申请的大小，系统会自动的将多余的那部分重新放入空闲链表中。
  - **申请大小的限制**（*非面试重点*）：栈是向低地址扩展的数据结构，是一块连续的内存的区域，栈顶的地址和栈的最大容量是系统预先规定好的，在 Windows 下，**栈的大小是 1M**，如果申请的空间超过栈的剩余空间时，将提示 overflow；堆是向高地址扩展的数据结构，是不连续的内存区域，这是由于系统是用链表来存储的空闲内存地址的，自然是不连续的，而链表的遍历方向是由低地址向高地址，堆的大小受限于计算机系统中有效的虚拟内存，32位 Windows 系统中，一个进程空间大小为 4G，其中进程的高位 2G 留给内核，低于 2G 留给用户，所以进程中的**堆的大小小于 2G**，进程的堆获得的空间比较灵活，也比较大。
  > 参考：[堆和栈的区别](https://zhuanlan.zhihu.com/p/78478567)，[进程空间分配和堆栈大小](https://www.cnblogs.com/ladawn/p/8449399.html)，[interview.md 堆和栈的区别]()

</details>
<details>
  <summary>对象复用的了解，零拷贝的了解</summary>

  - **对象复用**一般指继承和复合。public 继承塑膜出 is-a 关系，private 继承塑膜出根据某物实现关系；复合可以塑膜出 has-a 和根据某物实现关系。
  - **零拷贝**主要的任务是避免 CPU 将数据从一块存储拷贝到另一块存储。零拷贝技术常见于linux中，例如用户空间到内核空间的拷贝，可以采用零拷贝技术，通过 `mmap`，直接将内核空间的数据通过映射的方法映射到用户空间，即物理上共用这段数据。
    - **mmap**：
    - **sendfile**：
  >  参考：[interview.md 对象复用和零拷贝]()
...
<details> 
  ...
  - 内存池有多种实现方式，一般不建议自己实现内存池。
  - **固定内存池**
    - 应用程序先通过系统的内存分配调用预先申请适当大小的内存作为一个内存池，之后应用程序对内存的分配和释放都通过这个内存池来完成，当内存池大小需要扩展的时候，才需要再调用系统的内存分配。
    - 固定内存池由多个**固定大小的内存块**组成，每一个内存块又包含了**固定数量和大小的内存单元**。
    - 当有内存申请请求时，先遍历内存块链表，直到找到内存某个还有自由分配单元（*还没有被分配的内存单元*）的内存块，在该内存块中找到第一个自由分配单元，并返回该内存单元的起始地址。
    - 当某个内存单元进行回收时，该内存单元不会直接返回给堆，而是返回给内存池，遍历内存块链表，检查该内存单元是否在内存块中，将内存块中的该内存单元设置为空闲状态。
    - 由于固定内存池都是固定大小的内存块，如果程序员分配的内存大小大于内存池的单个内存块大小，应该怎么使用内存池，是新创建一个更大内存块大小的内存池吗，还是不使用内存池直接调用系统的内存分配完成这次的内存分配。
    - 固定内存池分配的内存应该从内存单元的起始地址开始，如果一个内存单元被分配但是没有被填满，这个内存单元的空闲位置是不能填入别的内容的，这样不还是会造成内存碎片吗，有的内存池可以合并内存，但是固定内存池应该不行，如果前一个内存的尾部位置位与内存单元的非尾部位置，那么后面的一个内存想和前一个内存合为一个内存，那么后一个内存的起始地址应为前一个内存的尾部位置的地址加一，但因为固定内存池分配的内存应该从内存单元的起始地址开始，所以这样合并两个内存应该是不行的。（*不确定*）
  - SGI STL 中的空间配置器（Allocators）使用了内存池的技术，SGI STL中使用了两级空间配置器，一级空间配置器就简单的把 `malloc`，`free`，`realloc` 包装了一下，二级空间配置器使用内存池进行内存管理。如果申请的内存大于 128 字节就将使用一级空间配置器进行管理，如果小于 128 字节就是用二级空间配置器进行管理，二级空间配置器将维护 16 个 `free-lists` 各自管理大小为 8，16，24，32，40，48，56，64，72，80，88，96，104，112，120，128 字节的内存块。
  ![avatar](./固定内存池.png)
  > 参考：[interview.md 内存池]()，[STL-空间配置器（allocators）](https://www.cnblogs.com/zyfgs2012/p/4100168.html)，[如何设计内存池](https://www.zhihu.com/question/25527491)

</details>
<details>
  <summary>C++ 中类对象的内存分配</summary>
...
<details>
  <summary>多重继承和虚继承</summary>

  - 

</details>
<details>
  <summary>拷贝构造函数为什么可以访问传入对象的私有变量</summary>

  -  你可以理解为同一个类的两个对象互为 friend，因此相互可以访问其私有变量？
  > 参考：[拷贝构造函数为什么可以访问传入对象的私有变量](https://blog.csdn.net/u012814856/article/details/79416627?utm_medium=distribute.pc_relevant.none-task-blog-title-3&spm=1001.2101.3001.4242)
...
<details>
  ...
  - **模板适用场景**
    ...
    - **类型适配 Traits（萃取）**：用 `Traits` 类表现类型信息，使用 `Traits` 类可以在**编译期间取得某些类型信息**。例子：当一个模板类需要模板参数类型的对象提供一个函数（如 `dosomething()` ）时，有的模板参数类型的对象可能有这个同名的函数，而有的类型可能没有，这样的话，如果有使用没有该同名函数的模板参数类型的模板，编译会通不过。为了解决这个问题可以增加一个 `Traits` 类，他会对外提供一个函数（如 `dosomething()`），对有该函数的模板参数类型，他转发这个函数；对没有这个函数的模板参数类型，他做了特化，实现一个空的函数，此时，无论是有该函数的模板参数类型还是没有该函数的模板参数类型都可以通过编译。
    ...
    ```
    - **元编程**：元编程的主要思想为，由于模板元编程（template meta program）执行于 C++ 编译期间，**可以将一些工作从运行期转移到编译期**。这样的话，一些在运行期才能检测到的错误在编译期就可以找到；同时，编译时间会变长，运行时间会变短。           
    ```C++
    // 利用元编程在编译期进行计算，以降低运行时间
    // 利用元编程计算阶乘，在编译阶段就完后阶乘的运算
    #include <iostream>
    using namespace std;

    template<unsigned n>
    struct Factional {
        enum { value = n * Factional<n - 1>::value }; // enum 是枚举常量表，里面元素都是常量，在编译期间就确定了其值
    };
    ...
    ```
  > 参考：[interview.md 模板]()，[C++ 模板总结](https://blog.csdn.net/tonglin12138/article/details/88595747)，[C++模板全特化、偏特化](https://blog.csdn.net/m_buddy/article/details/72973207)，[C++ 模板应用浅析](https://blog.csdn.net/cyxisgreat/article/details/37957687)，[Effective C++ 条款47-请使用 traits classes 表现类型信息]()，[Effective C++ 条款48-认识 template 元编程]()
...
<details>
  <summary>cookie 和 session</summary>
...
<details>
  ...
  - **数字签名原理**：首先，文件所有者利用散列算法（MD5 或 SHA-1） 将其原始文档转换为一个 16 字节长度的散列值，然后文件所有者利用他的**私钥**对该散列值进行运算得到数字签名。文件接收者接收到该文件、该文件的数字签名、文件所有者的公钥，文件接受者利用**公钥**对数字签名进行运算，可以得到该文件的散列值，文件接收者接着利用散列算法（与所有者用的一致）求出文档的散列值，判断两个散列值是否相同，若不相同说明文档修改过，若相同则说明文档没修改过。
  ![avatar](./数字签名.jpg)
  ...
  - **数字证书**：前面描述数字签名的原理时，文件所有者需要向文件接收者发送公钥，如何证明该公钥对应着该文件所有者，如何证明该公钥中途没有被修改过，答案是引入数字证书。文件所有者一般通过数字证书的方式发布公钥，数字证书一般包含用户姓名，公钥（*文件所有者的公钥*）和可信任的第三方数字签名，引入第三方数字签名是**为了保证公钥不会被篡改**，**证明这个公钥确实是该文件所有者的**。此时的公钥就如普通文件一样，利用数字签名的原理可保证公钥没被修改过，这里的第三方签名由认证机构（CA）提供，第三方数字签名的私钥由 CA 保管，公钥则向大众提供，用户即可利用其判断文件所有者提供的公钥是否正确。
...
<details>
  ...
  - **准备：DNS 和 ARP**
    ...
    - 用户设备生成一个目的地址为默认网关 IP 地址的 ARP 查询报文，该报文放入到一个具有广播码目的地址（FF: FF: FF: FF）的以太网帧中，并向交换机发送该以太网帧，交换机将该帧交付给所有连接的设备，包括网关路由器。
  ...
  - **Web 客户-服务器交互：TCP 和 HTTP**
    ...
    - 包含 HTTP 回答报文的数据报通过转发最终到达用户设备，用户设备的 Web 浏览器程序从套接字读取 HTTP 响应，从 HTTP 响应中抽取 Web 网页的 html，最终显示出网页。
...
<details>
  <summary>视频的时候卡顿，怎么确认是对方还是己方的网络卡顿</summary>
...
<details>
  ...
  - **布隆过滤器的应用场景**：针对海量数据的**查询**和**去重**。具体：网页黑名单，垃圾邮件过滤，url 去重，内容推荐。
...
<details>
  <summary>32 位操作系统和 64 操作系统</summary>
  
  
</details>
<details>
  <summary>32 位 CPU 和 64 位 CPU</summary>
  
  
</details>
<details>
  <summary>除法的效率</summary>
  
  
</details>
<details>
  <summary>调用系统调用 read 的过程</summary>
  
  > 参考：[深入剖析 Linux IO 原理和几种零拷贝机制的实现](https://juejin.im/post/6844903949359644680#heading-9)

</details>
<details>
  <summary>零拷贝</summary>
  
  > 参考：[深入剖析 Linux IO 原理和几种零拷贝机制的实现](https://juejin.im/post/6844903949359644680#heading-9)，[Linux 中的零拷贝](https://guodong.plus/2020/0802-153558/)

</details>
<details>
  <summary>一个程序从开始到结束的过程</summary>
...
<details>
  ...
  - 例子：创建一个操作符工厂，该工厂可以���产各种操作符实例，客户在生产前需要告知该工厂类需要生产的操作符实例类型。
  ```C++
  #include <iostream>
  #include <string>
  using namespace std;
...
<details>
  ...
  - **基本思路**：海图中岛屿通过有序的离散数据点构成的多边形来描述，描述一个岛屿的数据量十分大，所以使用基于面积的凸壳算法岛屿进行简化处理，降低描述岛屿的数据量；简化后的岛屿为凹多边形或者凸多边形，将其中的凹多边形分解为凸多边形；接着使用自由空间法把可行区域划分为多个多边形，多边形之间的连通关系使用图（数据结构）表示，图中的一个节点代表一个多边形，求起点到终点所经过的多边形，就是求有向图中两节点之间的可行路径；求得起点到终点所经过的多边形后，把多个多边形合并为一个多边形（这个多边形为一个可行通道），现在的任务转化为求多边形内两点的最短路径；可以使用 Dijkstra 算法计算两点的最短路径，但是使用 Dijkstra 计算出的路径，可能不符合实际的物理约束（最小航迹段长度、最大转弯角），可以引入稀疏 A* 算法解决此问题；稀疏 A* 算法使用 Dijsktra 算法计算出的路径作为其引导方向和启发信息，得到满足实际约束的路径。
  - **基于面积的凸壳算法**：
  - **自由空间法**：
  - **Dijkstra 算法**：
  - **稀疏 A 星算法**：
...
<details>
  ...
  - **信号和槽原理**：信号和槽其实就是**对象之间的一种通信机制**。要求两个对象（一个对象之间也可以，但没必要，信号和槽主要用来对象之间的通信）中的函数建立映射关系，**前者被调用时后者也要被调用**（前者是信号，后者为信号对应的槽函数）。
    ...
    ```
  - **事件系统**：一个事件是一个对象，所有的事件的祖先都来自 `QEvent`；只要有一个事件发生，就会有一个 `QEvent` 对象被被创建出来；可以通过 `public` 继承 `QEvent` 创建一个事件类；Qt 中有许多自带的事件类，如 `QMouseEvent` 等；每个事件类都有唯一的身份值 `type`；Qt
   提供了一个函数 `registerEventType()` 专门用于注册自定义事件，以保证事件身份值的唯一性；Qt 提供三个 `static` 函数用于发送事件，一个是直接发送 `sendEvent`，这种方式我们要等待对方处理，一个是发到队列 `postEvent`（Qt 程序中的 `main` 函数最后一行一般是 `return a.exec();`，这其实是开启了 Qt 的事件循环维护一个事件队列，`exec()` 的本质就是不停的从队列中获取事件来处理），这种方式将事件发送到事件队列中去，一个是在队列中立即处理 `sendPostedEvents`，这种方式的作用是将队列中的某个类型的事件立刻进行处理；所有的事件都会进入到 `QObject` 对象的 `event()` 函数，`event()` 函数本身不会去处理事件，而是根据事件类型调用不同的事件处理函数。
  - **Qt 中的元系统**：
  - **Qt 程序中类的关系**：

  > 参考：[Qt实用技能4-认清信号槽的本质](https://zhuanlan.zhihu.com/p/75126932)，[Qt 中的事件系统-有什么难的呢](https://zhuanlan.zhihu.com/p/50053079)

</details>
<details>
  <summary>通信问题</summary>
  
  - **通信流程**：通信模式是一对多的模式（*对方是主控模块，每隔一段固定的时间，就会给其他所有模块广播一个消息*），首先主控模块（*相当于所有模块的指挥中心*）使用 udp 广播一个包含初始序列号和其他初始信息的报文段给其他所有的模块，其他模块接收到该报文段以后，抽取出报文段中自己所需要的数据，然后计算出要返回给主控模块的数据，附加上序列号，通过 udp 再发送给主控模块，主控模块将其他模块发送来的数据放在一个缓存区中，然后按照顺序读取并处理缓存区中的数据，等到其他所有模块都返回一个报文段给主控模块，并且主控模块完成了所有数据处理的工作，并且到了广播报文段的时间，主控模块就会继续广播一个报文段给其他的所有模块，如此反复，直到主控模块广播一个结束报文段，其他模块接收到以后会返回一个 ACK 报文段。
  - **我这边与主控的通信流程**：对方会先发送一个起点和终点给我，接收到后我这边进行规划，规划完毕后返回第一个规划路径点给对方，对方接收到后根据这个规划路径点计算出实际路径点（*根据特定算法*），然后在下一次发送时间把这个实际路径点发送给我，我这边根据给定速度计算出靠近下一个规划路径点的路径点（*从实际路径点出发理论上可以靠近下一个规划路径点的路径*），并发送给对方，这样反复（*对方计算实际路径点，我这边计算靠近规划路径的可能路径点*），直到对方发送最后一个实际路径点，我这边将返回一个 ACK 报文。
  - **如何保证可靠数据传输**：参考了 rdt3.0 机制和 TCP 实现可靠数据传输的方式，主要在**应用层**使用序列号、校验和、超时重传（*发送方才有计时器*），缓冲区（*发送方才有缓冲区*）来实现可靠数据传输（*而 TCP 是在传输层实现可靠数据传输，程序员不需要干预*）。主控模块发送第一个报文段会包含一个初始序列号，然后主控模块每发送一个正常报文段都会将序列号加一，而且其他模块收到主控模块发送来的报文段后，要返回一个和主控模块报文段序列号一致的报文段给主控模块；当其他模块发现主控模块发送来的报文段校验不正确时，返回一个带有序列号的 NAK 报文段给主控模块，代表接收到的数据不正确，主控模块接着重新广播同序列号的报文段，已经接收到正确报文段的模块将忽略该报文段，而未接收到正确报文段的模块将接收该报文段；当主控模块发现其他模块发送来的报文段校验不准确时，重新广播同序列号的报文段，已经返回正确报文段的模块将忽略该报文段，而未返回正确报文段的模块将重新发送一个报文段给主控模块；当主控模块发送出报文段后一段时间（*自定义的*）没有收到其他所有模块返回的报文段，那么主控模块将重新广播该报文段，已经返回报文段的将忽略该报文段，未返回报文段的模块将返回一个报文段该主控模块；模块是怎么判断是否应该忽略主控模块重新发送来的报文段呢，报文段中会有一个 `char` 型（*8位*）的标识，每一位对应一个模块，1 代表可以接收，0 代表无需接收。
  - **为什么使用 udp 而不是 tcp**：因为项目中有一对多的通信场景，发送方是一个主控模块，给其他模块广播消息，其他模块从消息中抽取所需部分，tcp 不支持一对多通信，而 udp 支持一对多通信。
  - **为什么是停等模式**：因为主控模块每次广播完数据以后，需要等待所有模块返回一个报文段给主控模块，并且到达下一次发送时间后，主控模块才会进行下一次广播；而流水线模式是不需要等到上一个发出的报文段的对应 ACK 报文。
  - **应用层的报文段格式**：主控模块发送给其他模块的应用层报文中应该包括：序号，指定的接收模块的标识，具体数据，校验和；其他模块发送给主控模块的应用层报文中应该包括：序号，确认标识，具体数据，校验和。
...
<details>
  ...
  - **哪些地方得用多线程**：Qt 中使用多线程，一般就是为了不会有其他过程阻塞 ui 主线程，导致 ui 卡着不动。个人觉得 Qt 中应该不能有太多同时存在的线程，因为这样会分掉 ui 主线程的时间片，导致 ui 界面卡顿。项目中，ui 为一个线程，算法程序为一个线程，还有其他耗时比较长的地方都应该为一个线程。udp 通信需要为一个线程，虽然udp 发送数据这个动作耗时不长，udp 接收数据也不是使用轮询而是信号和槽机制，每当有数据报到来时，`QUdpsocket` 都会发射一个 `readyRead()` 信号，但是 udp 每次发送数据和接收数据，进行数据处理的时候相对耗时，所以 udp 通信需要为一个线程。
  - **线程间通信**：可以使用信号传递小数据，但最好不要用信号传递大数据，所以线程间通信使用全局变量。
  - **Qt 中线程操作**：有两种使用线程的方法，一种是继承 `QThread`，然后覆写 `virtual void run();` 函数，把耗时的操作放到 `run()` 函数中，然后在程序中使用一个线程对象的 `start()` 函数就可以另外开启一个线程，该线程将执行 `run()` 函数内的代码；还有一种方式是使用 `moveToThread()` 函数，如果一个类中有一个耗时函数，如果使用继承 `QThread` 的方法，将耗时函数放到新类的 `run()` 函数中，这将会破坏原来的类的结构，可以创建一个 `Qthread` 类对象，将含有耗时函数的对象 `moveToThread` 到这个 `QThread` 对象中，并建立信号和槽，当前类需要使用耗时函数的地方可以发送信号，而耗时函数代码放到槽函数中（*为什么要用信号和槽，而不是直接在需要使用耗时函数的时候，直接在这个地方调用这个函数，耗时函数所在的对象不是已经放到别的线程中去了吗，难道是直接调用就是在一个线程内执行，而使用信号通知那就是在其他线程中执行吗*）。
  > 参考：[Qt 多线程编程之敲开 QThread 类的大门](https://zhuanlan.zhihu.com/p/53270619)
...
<details>
  ...
  - **项目中的异常**
    - 航路规划模块的算法部分会提供一个公开的函数接口给使用者，当函数使用者输入的参数（*起始点，终止点，障碍物，规划区域*）不符合要求（*起止点和终止点不能在障碍物内，起始点和终止点不能在规划区域外，障碍物必须是多边形，规划区域必须是矩形*）时，该函数会抛出异常。算法模块中有些函数的输入参数如果不满足该函数规定的参数合法情况，也会抛出异常（*因为算法模块的有些错误输入参数在那个公开函数内设置检测条件很复杂*）。
    - 如果抛出标准库的异常，只能说明算法程序写的有问题，需要找出 bug 并进行修改。
...
<details>
  ...
  TEST(test2, c1) {
      EXPECT_EQ(4, add(1,2)); // 这是使用 EXPECT_EQ，由于这个断言是错误的，所以会有错误信息，而且还会继续执行下面的 ASSERT_EQ(6, add(1,2)); 断言
      ASSERT_EQ(6, add(1,2));
  }
  TEST(test3, c1) {
      ASSERT_EQ(6, add(1,2)); // 这是使用 ASSERT_EQ，由于这个断言是错误的，所以会有错误信息，但是由于使用的是 ASSERT_EQ，所以当前检测失败会退出该检测函数，不执行下面的 ASSERT_EQ(7, add(1,2)); 断言，但是这里只是退出这个函数，并不是退出整个程序，所以下面的 TEST(test4, c1) 还是会执行的
      ASSERT_EQ(7, add(1,2));
  }
  TEST(test4, c1) {
      ASSERT_EQ(5, add(2,3)); 
  ...
  ```
  - 仅仅是了解，用的不多，不主动介绍。
  > 参考：[gtest 的介绍和使用](https://blog.csdn.net/linhai1028/article/details/81675724)，[CLion 下的 gtest 测试](https://blog.csdn.net/weixin_38988633/article/details/92801084)，[记录搭一个ubuntu vscode + gtest 开发环境](https://zhuanlan.zhihu.com/p/141627260)
...
<details>
  ...
  - 给定一个场地，布置有多条白线、多个障碍物和多个目标，机器人需要沿着场地中的白线行驶，然后到达目标地点，执行指定的动作进行得分，在途中机器人可能需要跨越或者穿过各种障碍物。

</details>
<details>
  <summary>机器人程序流程</summary>

  - **容错机制**：当机器人的控制器，传感器，电机，电路板出现问题的时候，需要能够保证机器人能够继续正常运行。

</details>
<details>
  <summary>巡线</summary>
   
  - 巡线有两种策略，一个是使用线性 CCD 图像传感器，还有一个是使用巡线电路板。
  - **线性 CCD 图像传感器**
    - 一般的图像传感器采集的是二维图像，分辨率为 m * n，线性 CCD 图像传感器采集的图像是一维图像，分辨率是 m * 1。
    - 
  - **巡线电路板**
    - 
</details>
...
<details>
  <summary>取硬币问题</summary>
    
  - 30枚面值不全相同的硬币摆成一排，甲、乙两个人轮流选择这排硬币的其中一端，并取走最外边的那枚硬币。如果一方先取硬币，能保证得到的钱不会比对手少吗。
  - 先取者可以让自己总是取奇数位置上的硬币或者总是取偶数位置上的硬，数一数是奇数位置上的面值总和多还是偶数位置上的面值总和多，然后总是取这些位置上的硬币就可以了。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)

</details>
<details>
  <summary>蓝眼睛问题</summary>
    
  - 有个岛上住着一群人，突然规定，所有蓝眼睛的人都必须尽快离开这个岛，每晚8点会有一个航班离岛，每个人都看得见别人眼睛的颜色，但不知道自己的（别人也不可以告知），此外，他们不知道岛上到底有多少人是蓝眼睛的，只知道至少有一个人的眼睛是蓝色的，所有蓝眼睛的人要花几天才能离开这个岛。
  - 有多少个蓝眼睛的人就会花多少天。使用数学归纳，当蓝眼睛人个数为 1 时，蓝眼睛的人环视四周，发现没人是蓝眼睛，但至少有一个人是蓝眼睛，那么他就知道自己是那个蓝眼睛的人，所以他可以当天离开；当蓝眼睛人个数为 2 时，两个蓝眼睛的人看到对方，但他们不确定自己是不是蓝眼睛，也就是不确定蓝眼睛人的个数是 1 还是 2，他们知道如果蓝眼睛人个数为 1 的话，对方就会在第一天离开，但是发现第二天对方还在，因此知道了蓝眼睛人个数为 2，因此在第二天两个人就都会离开；当蓝眼睛人的个数为 3 时，这 3 人会意识到蓝眼睛人个数为 2 或者 3，如果蓝眼睛人的个数为 2，他们将在第二天离开，但是第三天另外两人如果还在岛上，那么三人就会意识到自己也是蓝眼睛人，第三天的时候三人就都会离开。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)

</details>
<details>
  <summary>蜡烛燃烧问题</summary>

  - 两根蜡烛，两端都有引线，燃烧完都需要 1 小时，怎么确定 15 分钟是多久。
  - 先点燃第一根的一端和第二根的两端，当第二根蜡烛燃烧完，代表过去了半个小时，接着点燃第一根的另一端，这根蜡烛烧完代表 15 分钟。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)

</details>
<details>
  <summary>乒乓球重量</summary>

  - 有 8 个乒乓球，其中一个更重，有一个秤，问至少几次能够找出更重的那个乒乓球。
  - 至少 2 次。将 8 个乒乓球分为 3 堆，分别有 3，3，2 个乒乓球。先秤个数为 3 和 3 的那两堆，如果一样重，代表更重的乒乓球在个数为 2 的那堆，接着从个数为 2 的那堆中拿一个秤，如果大于 3 个乒乓球的重量的三分之一，那么这个乒乓球就是更重的那一个，如果等于 3 个乒乓球的重量的三分之一，那么另一个乒乓球就是更重的那一个；先秤个数为 3 和 3 的那两堆，如果不一样重，说明重的乒乓球在个数为 3 的且更重的那一堆，3 个里面随便取 2 个，如果一样重，那么另一个乒乓球重，如果不一样重，那么更重的那个就是更重的乒乓球。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
  
</details>
<details>
  <summary>盐重量问题</summary>

  - 有 7g、2g 砝码各一个，天平一只，如何只用这些物品 5 次内将 140g 的盐分成 50g、90g 各一份。
  - 先将盐分为 70g 和 70g，然后通过 7g 和 2g 砝码将 70g 盐分为 9g 和 61g，再通过 9g 盐和 2g 砝码将 61g 盐分为 50g 和 11g，将 70g、9g、11g 合并为 90g 盐。（*可以利用已知重量的盐作为砝码*） 
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
...
<details>
  <summary>药丸问题</summary>

  - 有 20 瓶药丸，其中19瓶装有 1克/粒 的药丸，余下一瓶装有 1.1克/粒 的药丸。给你一台称重精准的天平，怎么找出比较重的那瓶药丸，天平只能用一次。
  - 从药瓶 1 中取出 1 粒药丸，从药瓶 2 中取出 2 粒药丸，从药瓶 3 中取出 3 粒药丸，以此类推，秤的总重量为 210 克（1 + 2 + 3 + ...... + 20 = 210），如有多出来的重量，那么一定来自 1.1 克的药丸，药瓶的编号可以算式 (weight - 210) / 0.1 求出，如药丸总重为 211.3 克，那么药瓶 3 装有较重的药丸。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
  
</details>
<details>
  <summary>概率问题 1</summary>

  - 一个家庭中有两个小孩，其中一个是女孩，问另一个也是女孩的概率。
  - 条件概率问题。P(已知两个孩子中有一个是女孩) = 个数((男女), (女男), (女女)) / 个数((男男), (男女), (女男), (女女)) = 3/4；P(两个都是女孩) = 个数((女女)) / 个数((男男), (男女), (女男), (女女)) = 1/4；P(两个都是女孩 | 已知两个孩子中有一个是女孩) = (1/4) / (3/4) = 1/3。 
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
  
</details>
<details>
  <summary>概率问题 2</summary>

  - 你有两个罐子，每个罐子各有若干红色弹球和蓝色弹球，两个罐子共有50个红色弹球，50个蓝色弹球，随机选出一个罐子，随机从中选取出一个弹球，要使取出的是红球的概率最大，一开始两个罐子应放几个红球，几个蓝球，在你的计划中，得到红球的准确几率是多少。
  - 一个罐子放置 1 个红色弹球，另外一个罐子放置 49 个红色弹球和 50 个篮色弹球，P(取到红球) = P(在第一个罐子中取球) * P(在第一个罐子中取到红球) + P(在第二个罐子中取球) * P(在第二个罐子中取红球) = (1/2) * (1/1) + (1/2) * (49/99) = 0.747。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
  
</details>
<details>
  <summary>扑克牌问题</summary>

  - 有 54 张扑克牌，其中有 10 张是翻过来的，现在把你的眼睛蒙上，让你把扑克牌分成两叠（两叠的多少可以不一样），要求在两叠中翻过来的扑克牌是相等的，请问该怎么做。
  - 从 54 张扑克牌中任意取出 10 张，现在，扑克牌被分成了两叠，44 张和 10 张，然后将 10 张的那一叠的每一张都翻过来，而 44 张的那一叠不动，这样的话就得到了翻过来的扑克牌是相等的两叠牌。证明：假设 44 张那叠牌中正面牌是 x 张，10 张那叠牌中是 10-x 张，然后将 10 张那叠每一张都翻过来，那么反面牌将为 10-x 张，正面牌为 x 张，和 44 张的那叠的正面牌个数一样。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
  
</details>
<details>
  <summary>扔鸡蛋问题</summary>

  - 有栋建筑物高 100 层。若从第 N 层或更高的楼层扔下来，鸡蛋就会破掉，若从第 N 层以下的楼层扔下来则不会破掉，给你 2 个鸡蛋，请找出 N，并要求最差情况下扔鸡蛋的次数为最少。
  - 因为只给 2 个鸡蛋，所以不能用二分法，破了就没法用了。可以试着从 10 层开始扔鸡蛋，然后是 20 层，直到鸡蛋 1 破了，然后就可以把楼层范围确定在 10 层范围内，比如 20 层没碎，30 层碎了，那么楼层在 20 ~ 30 层内；让鸡蛋 1 第一次从 10 楼扔下，如果鸡蛋 1 破了，那么最多还需要再扔 9 次（9, 8, ... , 1）鸡蛋 2，那么最差总共扔了 9+1=10 次；如果鸡蛋 1 一直到 100 层才碎，那么最多还需要再扔 9 次（99, 98, ..., 91）鸡蛋 2，那么最差总共扔了 10+9=19 次。这种方法的话最差的情况也有不同的情况，可以对其进行“负载均衡”，使得其所有的最差情况都是一样的情况，求解方程 x+(x-1)+(x-2)+...+1=100，得出 x=14，我们可以从 14 层开始扔鸡蛋 1，然后是 27（27-14=13）层，接着是 39（39-27=12）层，以此类推，如果 14 层没破，27 层破了，那么最差需要扔 2+12（14, 27, 15, ... , 26）次，如果 27 层没破，39 层破了，那么最差需要扔 3+11（14, 27, 39, 28, ..., 38）次，实现了“负载均衡”。
  ![avatar](./扔鸡蛋问题.jpg)
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
  
</details>
<details>
  <summary>水果标签问题</summary>

  - 3个箱子里面放了苹果，梨子，苹果加梨子，标签全错误（这里的标签是箱子上的标签，不是指水果上的标签），只能选择查看一箱的水果来改正所有标签。
  - 查看贴着苹果和梨标签的那一箱，如果里面的是苹果，代表这一箱只有苹果，因为标签都错了，这个箱子贴着的标签是苹果和梨，所以不可能是苹果和梨，所以这一箱全是苹果，剩下的两箱一个贴着苹果，一个贴着梨，而且实际上这两箱中的一个是苹果和梨，一个是梨，由于标签是错误的，贴着梨的里面不可能是梨，所以贴着梨的箱子里面放的是苹果和梨，那么贴着苹果的里面放的是梨；查看贴着苹果和梨标签的那一箱，如果里面的是梨，代表这一箱只有梨，因为标签都贴错了，这个箱子贴着的标签是苹果和梨，所以不可能是苹果和梨，所以这一箱全是梨，剩下的两箱中一个贴着苹果，一个贴着梨，而且实际上这两箱中的一个是苹果和梨，一个是苹果，由于标签是错误的，贴着苹果的里面不可能是梨，所以贴着苹果的箱子里面放的是苹果和梨，那么贴着梨的箱子的里面放的是苹果。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)，[水果问题](https://blog.csdn.net/qq_43606352/article/details/98898786)
  
</details>
<details>
  <summary>吃药问题</summary>

  - 你每天需要同时服用 A、B 两种药片各一颗，不能多也不能少，你打开装药片 A 的药瓶，倒出一粒药片放在手心，然后打开另一个药瓶，但不小心倒出了两粒药片。现在，你手心上有一颗药片 A，两颗药片 B，并且你无法区别哪个是 A，哪个是 B，你如何才能严格遵循药方服用药片，并且不能有任何的浪费。
  - 把手上的三片药各自切成两半，分成两堆摆放。再取出一粒药片A，也把它切成两半，然后在每一堆里加上半片的A。现在，每一堆药片恰好包含两个半片的A和两个半片的B。一天服用其中一堆即可。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
  
</details>
<details>
  <summary>硬币问题</summary>

  - 如何用一枚硬币等概率地产生一个1到3之间的随机整数，如果这枚硬币是不公正的呢。
  - 如果是公正的，投掷两次，“正反”为 1，“反正”为 2， “正正”为 3，“反反”重来；如果不是公正的，投掷三次，令“正反正反”为 1，“反正正反”为 2，“正反反正”为 3，其余情况重来。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
  
</details>
<details>
  <summary>灯管问题</summary>

  - 在房里有三盏灯，房外有三个开关，在房外看不见房内的情况，你只能进门一次，你用什么方法来区分那个开关控制那一盏灯。 
  - 打开一个开关，过 10 分钟后关掉开关，并打开另一个开关，进入房间确认，亮的灯是由第二次打开的开关控制的，摸上去发热但不发亮的灯是由第一次打开的开关控制的，剩下的那盏灯由未操作过的开关控制。
  > 参考：[程序员常见智力题 30 道](https://www.nowcoder.com/discuss/414594)
...
## 场景题
<details>
  <summary>怎么设计一个高并发的 tcp 服务器</summary>
  
  
</details>
<details>
  <summary>怎么设计一个秒杀系统</summary>
...
<details>
  ...
  - 自主性比较强，本科的时候因为参加机器人比赛，期间自学了嵌入式和自动控制的相关内容；然后因为对控制算法比较感兴趣，跨考到控制工程，期间也是自学了一些自动化的专业课；然后研究生期间因为实验室项目需要，也自学了计算机相关的基础知识；这些的话都是自己主动参加和学习的，我觉的显示了自己的一个主动性。

</details>
<details>
  <summary>缺点</summary>
  
  - 很容易紧张，在人多的情况下表达自己的观点时会很紧张，导致表达不流畅。

</details>
<details>
  <summary>职业规划</summary>
  
  - 我觉得在入职前几年，首先需要先熟练掌握工作必备的一些基础知识和技能，在实际项目中深入理解和运用这些这些技能。
  - 然后对工作的整体框架能有自己的认识，然后进一步锻炼自己的表达能力和理解能力，可以将需求转换为实际代码，争取能成为一个项目的主导者。

</details>
<details>
  <summary>岗位理解</summary>
  
  - 我觉的后端开发主要有网络通信，数据存储，逻辑处理这几个方面。
  - 网络通信的话就是得保证网络的可靠性，低延时。
  - 数据存储的话就是得保证数据的可靠性（备份），高性能（缓存）。
  - 逻辑处理的话就是得保证满足给定需求，并且没有 bug。

</details>
<details>
  <summary>为什么考研跨考</summary>
  
  - 因为本科期间在机器人实验室学习了机器人相关的知识，觉得控制算法这块比较有趣，所以考研的时候就选择了控制工程。

</details>
<details>
  <summary>研究生研究方向</summary>
  
  - 航路规划，冲突检测。
  - 主要背景是在联合作战过程中，不同部队的指挥员根据作战任务需求制定作战任务规划方案，由于不同部队的指挥员一般只了解自己所在团队的情况，没考虑不到和其他部队的规划情况，所以在这种情况下作战任务之间很容易产生冲突，所以需要对这些任务集合进行冲突检测，发现这些冲突以后，再形成符合作战需求的整体任务规划方案。
  - 主要工作就是针对任务规划过程中可能产生的时间冲突，时空冲突，资源冲突进行统一的建模，并对各类冲突进行检测。
  - 主要思路是基于 STN 网拓展他能表示的信息，使得可以表示这几种不同的冲突信息。

</details>
<details>
  <summary>介绍一下项目</summary>
  
  - **航路规划项目介绍**：给定一张海图以及起点和终点，要规划出一条从起点到终点的可行路径，并开发用于检验算法可行性的桌面软件；算法主要难点就是要保证效率和满足实际的物理约束；针对效率问题，就使用凸壳算法对岛屿的数据描述量进行简化，使用自由空间法将可行区域划分为多条通道；针对物理约束问题，就使用稀疏 A 星算法代替迪杰斯特拉算法，引入物理约束进行规划。
  - **航路规划项目中遇到的困难**：算法设计上，保证效率和物理约束；软件设计上，算法有很多边界问题需要考虑，一开始写代码的时候没有考虑边界情况，自己测试的时候没有发现问题，然后师兄来进行测试的时候，就发现了很多边界情况都没有考虑到，后来就正对每个函数都设计了测试案例，考虑了一些极限的情况进行测试，然后进行调试和修改，对整个算法模块的话也用 Qt 设计的桌面软件进行功能测试，保证整个算法模块的一个可靠性。
  - **机器人项目介绍**：比赛给定一个场地，布置有多条白线、多个障碍物和多个目标，机器人需要沿着场地中的白线行驶，然后到达目标地点，执行指定的动作进行得分，在途中机器人可能需要跨越或者穿过各种障碍物；我主要负责机器人程序的编写和机器人的调试；程序功能主要有：巡线，电机控制，舵机控制，传感器状态获取，机器人姿态获取和调整。
  - **机器人项目遇到的问题**：比赛前几个星期公布正式的比赛场地，由于机器人是针对原来的场地设计的，而且调试也是在以前的场地上，在这几个星期里需要根据实际的比赛场地，在原有机器人的基础上进行重新设计和调试。
  - **从项目中学习到了什么**：**学会了怎么从知识转换为实践**，由简到繁，由易到难，本科期间机器人项目需要用到很多不同的模块，但是我用的那款单片机资料很少，大部分模块的驱动代码都得自己从头开始写，所以我一般是在一个开源硬件 Arduino 上实现驱动程序，然后参考这个驱动程序和单片机的数据手册，将驱动移植到我使用的那款单片机上，研究生期间的话主要就是需要从最简单的算法开始学起，翻阅大量的论文，重新推导一遍算法，然后用程序实现算法功能，对算法进行比较，然后选择更为有效的算法继续进行研究，**学会了团队合作**，本科机器人比赛中机械，硬件，软件必须相互协作，研究生项目中也需要明白导师的需求，还需要和同学确定接口，网络通信，代码风格的统一性。

</details>
<details>
  <summary>工作地点的选择</summary>
  
  - 希望能找到一份在深圳的工作，我家在江西赣州，离深圳很近，然后明年还会开通到深圳的高铁，这样从我家到深圳的话只要 2 个小时左右。

</details>
<details>
  <summary>对之前面试官的评价</summary>
  
  - 之前的面试官都很专业，而且都很有耐心，遇到不熟悉的知识点会慢慢引导我，在讨论项目的过程中也发现了自己项目中一些原来没有考虑到的点，我觉得两次面试的体验都很好。

</details>
<details>
  <summary>非科班是怎么学习的</summary>
  
  - 有项目的时候，根据项目中的关键点进行延申，然后学习。
  - 没有项目的时候，就看一些经典的书籍，遇到不理解的知识点就通过看视频和看博客解决。

</details>
<details>
  <summary>最有成就感的事情</summary>
  
  - 本科期间参加的中国机器人大赛结果比较好；
  - 在发布正式比赛场地前做的准备比较充分，可以在比赛前在针对新的场地更新机器人的设计；
  - 团队沟通比较有效，在现场的比赛过程中出现问题可以快速解决。

</details>
<details>
  <summary>最有挫败感的事情</summary>
  
  - 高考没考好。
  - 没有阶段性的总结。
  - 没有把握好优先级，把时间花在了性价比不高的地方，没有针对自己的弱项进行攻克。

</details>
<details>
  <summary>为什么选择后端方向</summary>
  
  - 觉得后端开发比较有前景，有明确的成长路线，可以从初级程序员向高级程序员或架构师发展。
  - 学习的一些知识和技能比较契合后端这个岗位。
  - 性格比较安静内敛，适合从事技术类的职业。

</details>
<details>
  <summary>第一份工作想得到什么</summary>
  
  - 希望自己的专业技能可以得到快速的提升。
  - 希望自己在为人处世这方面能得到一定的锻炼。

</details>
<details>
  <summary>为什么选择虾皮</summary>
  
  - 我想做电商相关的工作。（想做面试公司相关业务的工作）
  - 觉得虾皮前景很好，电商业务现在越来越火，很贴近生活，而且虾皮做的是跨境电商，我觉得在全球化的环境下发展前景会很好。（面试公司的前景好，举例）
  - 面试体验很好，觉得面试官很专业很有耐心，我觉得在这样的团队中工作可以学习到很多，工作氛围也会很好。
  - 虾皮的技术比较新，希望去一个技术氛围比较好的公司。
  - 虾皮地点在深圳，我家在江西赣州，离深圳很近，然后明年还会开通到深圳的高铁，这样从我家到深圳的话只要 2 个小时左右。（珠三角的说离家近，长三角的说希望来这块发展，考研的时候就考虑到了以后要在长三角发展）

</details>
<details>
  <summary>如果都发了 offer，怎么选择</summary>
...
<details>
  <summary>对虾皮的了解</summary>
  
  - 了解到虾皮的主要业务是跨境电商，我目前知道虾皮的几个部门有做电商，金融和音视频。（需要调研）

</details>
<details>
  <summary>投递了哪些公司，这些公司的特点</summary>
  
  - 这些公司的特点是目前都在快速发展。

</details>
<details>
  <summary>理想中的团队是怎么样的</summary>

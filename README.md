...
<details> 
  ...
  - **固定内存池**
    ...
    - 内存池分配的内存必须从内存单元的起始地址开始，如果一个内存单元被分配但是没有被填满，这个内存单元的空闲位置是不能填入别的内容的，这样不还是会造成内存碎片吗，有的内存池可以合并内存，但是固定内存池应该不行，因为他的最小操作单元应该就是一个固定大小的内存单元。
...
<details>
  <summary>Boost 库</summary>

</details>
<details>
  <summary>C++ 中的 </summary>
...
<details>
  ...
  - 准备看《Linux 多线程服务端编程 使用 muduo C++ 库》
  > 参考：[Linux 多线程服务端编程 使用 muduo C++ 库]()

</details>
<details>
  <summary>对你提升最大的一本书</summary>
  
  - 《Accelerated C++》或者《Effetive C++》
  > 参考：[Accelerated C++]()，[Effetive C++]()
